...
def run_button1_calibration(paths, dry_run=False):
    ...
    open_vasm_from_read_dialog(paths, dry_run)

    asx_hwnd = wait_for_window(ASX_TITLE, timeout=20)
    activate_window(asx_hwnd, target_size=ASX_TARGET_SIZE)
    click_reference(asx_hwnd, ASX_REFERENCE_SIZE, EDIT_BUTTON, dry_run, "ASX Edit")
    LOGGER.screenshot("after_edit_click")

    setting_hwnd = wait_for_window(SETTING_TITLE, timeout=20)

...
import button1_calibration as common
...
def prompt_sample_count(default_value=180):
    root = tk.Tk()
    root.withdraw()
    ...
    while True:
        raw_value = simpledialog.askstring(
            "Sample Count",
            "Enter sample count. (1 ~ 240, blank uses 180)",
            parent=root,
        )
        ...
        if not raw_value:
            root.destroy()
            return default_value
        if raw_value.isdigit() and 1 <= int(raw_value) <= 240:
            root.destroy()
            return int(raw_value)
        messagebox.showerror("Invalid Input", "Enter a number from 1 to 240.", parent=root)
...
def run_button2_sample_measurement(paths, sample_count, dry_run=False, launcher_path=None):
    ...
    common.open_vasm_from_read_dialog(paths, dry_run)

    asx_hwnd = common.wait_for_window(common.ASX_TITLE, timeout=20)
    common.activate_window(asx_hwnd, target_size=common.ASX_TARGET_SIZE)
    common.click_reference(asx_hwnd, ASX_REFERENCE_SIZE, common.EDIT_BUTTON, dry_run, "ASX Edit")

    setting_hwnd = common.wait_for_window(common.SETTING_TITLE, timeout=20)
...
def parse_args():
    ...
    parser.add_argument(
        "--sample-count",
        type=int,
        help="Measurement sample count. If omitted, a prompt opens. Blank input defaults to 180.",
    )
    parser.add_argument(
        "--source-vasm",
...
def main():
    ...
    try:
        ...
        sample_count = args.sample_count if args.sample_count is not None else prompt_sample_count()
